...
contract Markets4Cast is Ownable {
    ...
    function limitBuy(
        ...
        uint256 size,
        Outcome outcome
    ) external marketActive(marketId) returns (bytes32 orderId) {
        ...
        LibBitmap.Bitmap storage ticks = outcome == Outcome.Yes
            ? market.yesUnifiedTicks
            : market.noUnifiedTicks;
        ...
        priceLevel.orders.push(
            LimitOrder({maker: msg.sender, size: size, side: Side.Bid})
        );
    ...
    function marketBuy(
        ...
        uint256 size,
        Outcome outcome
    ) external marketActive(marketId) returns (uint256 fulfilled) {
        if (size == 0) revert InvalidSize();
        ...
        LibBitmap.Bitmap storage counterTicks = outcome == Outcome.Yes
            ? market.noUnifiedTicks
            : market.yesUnifiedTicks;
        LibBitmap.Bitmap storage counterBidTicks = outcome == Outcome.Yes
            ? market.noBidTicks
            : market.yesBidTicks;
    ...
    function limitSell(
        ...
        LibBitmap.Bitmap storage ticks = outcome == Outcome.Yes
            ...
            : market.yesUnifiedTicks;
        PriceLevel storage priceLevel = outcome == Outcome.Yes
            ? market.noOrders[tickPrice]
            : market.yesOrders[tickPrice];

        ticks.set(tickPrice);
        priceLevel.orders.push(
            LimitOrder({maker: msg.sender, size: size, side: Side.Ask})
        );
        priceLevel.totalSize += size;
        ...
        while (true) {
            ...
            for (
                ...
                SafeTransferLib.safeTransfer(
                    collateral,
                    msg.sender,
                    (counterFulfilled * marketPrice * _collateralMultiplier) /
                        BPS
                );
    ...
    function cancelOrder(
        ...
        Side side,
        Outcome outcome
    ) external marketActive(marketId) {
        Market storage market = markets[marketId];
        ...
        if (side == Side.Bid) {
            ...
            uint256 collateralRefund = (order.size *
                price *
                _collateralMultiplier) / BPS;
            order.size = 0;
            ...
            priceLevel.totalSize -= order.size;
            mapping(address => uint256) storage balances = outcome ==
                Outcome.Yes
                ? market.yesBalances
                : market.noBalances;
        ...
        bytes32 orderId = side == Side.Bid
            ? getOrderId(marketId, price, orderIndex)
            : getOrderId(marketId, BPS - price, orderIndex);
    ...
    function claim(uint256 marketId) external {
        ...
        mapping(address => uint256) storage balances = market.outcome ==
            Outcome.Yes
            ? market.yesBalances
            : market.noBalances;
